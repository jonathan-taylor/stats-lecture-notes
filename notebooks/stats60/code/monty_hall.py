...
host_html = ('<img height="%d" width="%d" src="data:image/png;base64,%s">' % 
             (height, 
              width,
              encodestring(file(os.path.join(imgdir, 'host.png')).read()).decode('ascii')))

student_html = ('<img height="%d" width="%d" src="data:image/png;base64,%s"/>' % 
                (height,
                 width,
                 encodestring(file(os.path.join(imgdir, 'user-student.png')).read()).decode('ascii')))

student_win_html = ('<img height="%d" width="%d" src="data:image/png;base64,%s"/>' % 
                    (height,
                     width,
                     encodestring(file(os.path.join(imgdir, 'student_win.png')).read()).decode('ascii')))

student_lose_html = ('<img height="%d" width="%d" src="data:image/png;base64,%s"/>' % 
                     (height,
                      width,
                      encodestring(file(os.path.join(imgdir, 'student_lose.png')).read()).decode('ascii')))

car_html = ('<img height="%d" width="%d" src="data:image/png;base64,%s"/>' % 
            (height,
             width,
             encodestring(file(os.path.join(imgdir, '310px-Car_with_Driver-Silhouette.svg.png')
                                         ).read()).decode('ascii')))
